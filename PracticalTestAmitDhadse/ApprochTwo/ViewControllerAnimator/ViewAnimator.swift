...
final class ViewAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    init?(type: PresentationType, firstViewController: FirstViewController, secondViewController: SecondViewController, selectedCellImageViewSnapshot: UIView) {
        ...
        cellImageViewRect = selectedCell.iBackGroundView.convert(selectedCell.iBackGroundView.bounds, to: window)
    }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard
            ...
            let cellImageSnapshot = selectedCell.iBackGroundView.snapshotView(afterScreenUpdates: true),
            let controllerImageSnapshot = secondViewController.iGradientView.snapshotView(afterScreenUpdates: true),
            let closeButtonSnapshot = secondViewController.iCloseButton.snapshotView(afterScreenUpdates: true)
        else {
        ...
        let controllerImageViewRect = secondViewController.iGradientView.convert(secondViewController.iGradientView.bounds, to: window)
        let controllerLabelRect = secondViewController.iLabel.convert(secondViewController.iLabel.bounds, to: window)
        let closeButtonRect = secondViewController.iCloseButton.convert(secondViewController.iCloseButton.bounds, to: window)
